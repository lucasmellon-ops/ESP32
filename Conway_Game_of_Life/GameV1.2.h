...
const int BUTTON_PIN = 0;  // IO00 pin

#define NUM_LEDS 1
#define DATA_PIN LED_DI_PIN  // IO40
#define CLOCK_PIN LED_CI_PIN  // IO39
...
void showEntropyLED() {
  ...
  float alivePercentage = float(aliveCount) / ((WIDTH / CELL_SIZE) * (HEIGHT / CELL_SIZE));
  
  if (alivePercentage < 0.5) {
    // gradient from red to yellow
    leds[0] = CRGB(255, 255 * (alivePercentage * 2), 0);
  } else {
    // gradient from yellow to green
    leds[0] = CRGB(255 * (2 - 2 * alivePercentage), 255, 0);
  }
